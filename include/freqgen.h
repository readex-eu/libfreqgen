...
{
    char* name;
    int (*get_num_devices)(); /** < returns the number of available devices (e.g., number of CPUs,
                                 uncores) */
    /**
     ...
     */
    freq_gen_setting_t (*prepare_set_frequency)(
        long long int target, int turbo); /** < prepare a specific frequency for a device */
     ...
     * If it does not exist, the interface is either not there or not implemented and the frequency
     * is not considered to be a range.
     * @param fp from init_device
     * @return frequency in Hz or an error (<0)
     */
 ...
 * @param type get a handle for core or uncore frequency. The function will iterate over some
 * implementations and return the first suitable.
 * You may try to open a core/uncore device to check whether this works immediately after. If not,
 * you can finalize the interface and get the next one.
 * When there are no more suitable interfaces, this function returns NULL. When calling it again
 * after receiving NULL, it will also start trying out the interface again from the beginning.
